...
if (class_exists('xml')) return;
	
	// PHP 5.2.6 broke XML parsing -- I 'fixed' it by substuiting 
	// in placeholders and then cleaning it up later



function fixup567Behavior(&$in) {
	static $_escapedTermsForBroken = array("&gt;","&lt;","&amp;");
	static $_realTermsForBroken= array(">","<","&");
	static $_interTermsForBroken = array("|||gt;","|||lt;","|||amp;");
          

     
	if ($in['tag']=='content' || $in['tag']=='title') {
     	$in['value']=str_replace($_interTermsForBroken,$_realTermsForBroken,$in['value']);
	} else if ($in['tag']=='link') {
		$in['attributes']['href']=str_replace($_interTermsForBroken,$_realTermsForBroken,$in['attributes']['href']);
	}
...
class xml  {
   ...
   function parse($xml,$skipWhitespace=1)
   {
	static $_escapedTermsForBroken = array("&gt;","&lt;","&amp;");
	static $_realTermsForBroken= array(">","<","&");
	static $_interTermsForBroken = array("|||gt;","|||lt;","|||amp;");
    $this->_parser = xml_parser_create("UTF-8");
       		...
       		// PHP 5.2.6 broke xml parsing http://bugs.php.net/bug.php?id=45996
		// this is the workaround
	   if (version_compare(phpversion(),"5.2.5",">=")) {
       		$xml=str_replace($_escapedTermsForBroken,$_interTermsForBroken ,$xml);

       		 }
       $this->input = $xml; // This really confuses things to utf_decode it... Dont do itutf8_decode($xml);// Unicode Support 
	   /* Remove the following two lines since for rapidblog we don't want to escape HTML */
      ...
      // $xml = str_replace(">{lf}", ">\n", $xml);
       ...
       xml_parser_set_option($this->_parser, XML_OPTION_SKIP_WHITE, $skipWhitespace);
       
      if ( xml_parse_into_struct($this->_parser, $xml, $this->_struct, $this->_index))

		// PHP 5.2.6 broke xml parsing http://bugs.php.net/bug.php?id=45996
		// this is the workaround
	   if (version_compare(phpversion(),"5.2.5",">="))  {
            array_walk($this->_struct,"fixup567Behavior");
